...
static int sl_hashtable_resize(sl_hashtable *hashtable, size_t size)
{
    ...
    for (size_t n = 0; n < hashtable->size; n ++) {
        ...
        for (sl_hashtable_bucket *node = bucket; node != NULL; node = node->next) {
            ...
            sl_hashtable_bucket *last_node = new_bucket;
            for (sl_hashtable_bucket *new_node = new_bucket; new_node != NULL; new_node = new_node->next) {
                last_node = new_node;
            }
...
static bool sl_hashtable_needs_resize(sl_hashtable *hashtable) {
    return hashtable->count * 100 / hashtable->size >= SL_HASHTABLE_MAX_LOAD;
}

void sl_hashtable_init(sl_hashtable *hashtable, sl_arena *arena, size_t preallocate)
{
